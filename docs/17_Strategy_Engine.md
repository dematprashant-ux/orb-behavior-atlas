...
M28.1 adds `OptimizationRunIdentity`, a caller-supplied immutable non-blank
string value for identifying completed optimization runs at a future storage
boundary. `IdentifiedOptimizationRun` retains the exact supplied identity and
the exact immutable `OptimizationRun` by reference. Identity remains separate
from execution results: no identifier is generated by `OptimizationRun`, and
this boundary introduces no repository contract, persistence technology,
serialization, I/O, reporting, or optimization behavior.
