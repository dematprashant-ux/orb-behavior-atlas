"""Contract tests for deterministic Markdown optimization summary rendering."""
...
class MarkdownOptimizationRunSummaryReportRendererTests(TestCase):
    """Verify Markdown rendering reads only retained scalar analysis values."""
    ...
    def test_renderer_is_stateless_and_satisfies_the_existing_protocol(self) -> None:
        ...
        self.assertEqual(renderer, MarkdownOptimizationRunSummaryReportRenderer())
        self.assertEqual(
            repr(renderer),
            repr(MarkdownOptimizationRunSummaryReportRenderer()),
        )
        self.assertEqual(MarkdownOptimizationRunSummaryReportRenderer.__slots__, ())

    def test_rendering_uses_the_exact_heading_table_order_and_raw_values(
        self,
    ) -> None:
        report = _report(
            (
        ...
        rendered = MarkdownOptimizationRunSummaryReportRenderer().render(report)
    ...
    def test_empty_report_renders_heading_table_and_all_zero_metric_rows(self) -> None:
        rendered = MarkdownOptimizationRunSummaryReportRenderer().render(_report(()))
    ...
    def test_rendering_does_not_recalculate_traverse_or_call_plain_text(
        self,
    ) -> None:
        ...
        with patch.object(
            ...
            first = MarkdownOptimizationRunSummaryReportRenderer().render(report)
            second = MarkdownOptimizationRunSummaryReportRenderer().render(report)
    ...
    def test_invalid_report_inputs_are_rejected_without_partial_output(self) -> None:
        renderer = MarkdownOptimizationRunSummaryReportRenderer()
        report = _report(())
    ...
    def test_public_export_is_intentional(self) -> None:
        from src.engines.backtesting import (
            MarkdownOptimizationRunSummaryReportRenderer as PackageRenderer,
        )
        from src.engines.backtesting.reporting import (
            MarkdownOptimizationRunSummaryReportRenderer as ModuleRenderer,
        )

        self.assertIs(PackageRenderer, MarkdownOptimizationRunSummaryReportRenderer)
        self.assertIs(ModuleRenderer, MarkdownOptimizationRunSummaryReportRenderer)
